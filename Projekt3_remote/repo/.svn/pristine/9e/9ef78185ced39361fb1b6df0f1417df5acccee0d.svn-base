...
public class MoveToClickInput : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            if (!EventSystem.current.IsPointerOverGameObject()) //! prevent character to walk when clicking on inventory
            {
                ...
                var playerFeetTransform = transform.position - new Vector3(0, 1, 0);
                RequestPathManager.RequestPath(playerFeetTransform, targerPosition, OnPathFound);
            }
        }
    ...
    {
        var playerFeetTransform = transform.position - new Vector3(0, 1, 0);
        RequestPathManager.RequestPath(playerFeetTransform, targetPosition, OnPathFound);       
    }
